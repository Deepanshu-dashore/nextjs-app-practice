...
export default function InsightsSection() {
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? blogPosts.length - cardsPerView : prev - 1
    );
  };

  const nextSlide = () => {
    setCurrentIndex((prev) =>
      prev >= blogPosts.length - cardsPerView ? 0 : prev + 1
    );
  };
  ...
  <div className="flex justify-end gap-3 sm:gap-4 mt-6 sm:mt-8">
    <button
      onClick={prevSlide}
      className="border px-3 sm:px-4 py-2 rounded-md text-white hover:bg-purple-500"
    >
      <ChevronLeft />
    </button>
    <button
